...
- [x] 11.4 사이드바 활성 상태 동적 하이라이트. **완료 기준**: `/audit/preview` 방문 시 감사 로그 행이 하이라이트. (오케스트레이터가 Java 파일 1개 추가를 예외로 승인: `com.hospitalops.web.CurrentUriModelAdvice`(`@ControllerAdvice` + `@ModelAttribute("currentUri")`)로 모든 화면에 현재 요청 URI를 주입하고, sidebar 5개 실 라우트에 `th:classappend="${currentUri == '...'} ? 'is-active' : ''"` 적용. 실제 MySQL로 `AuditLogControllerIT`(10/10) 재확인, `/audit/preview` 응답 HTML 실측: sidebar-item 12개 중 `is-active` 정확히 1개, 감사 로그 링크에만 부착)
- [x] 11.5 재사용 콘텐츠 셸 프래그먼트(인사말/4카드/3단그리드/AI패널 크롬). **완료 기준**: 구조 사실 9~14 존재. (프래그먼트만 정의 — 11.6~11.8에서 실제 페이지에 끼워 넣을 때 최종 확인)
- [x] 11.6 `audit/list.html` 리스킨. **완료 기준**: `AuditLogControllerIT` 무수정 통과. (오케스트레이터가 로컬 native MySQL로 실행해 확정: tests=10 failures=0 errors=0. 11.4 반영 후 재실행으로도 재확인)
- [x] 11.7 `reports/patient-visit-summary.html` 리스킨. **완료 기준**: `PatientVisitSummaryControllerIT` 무수정 통과. (오케스트레이터가 로컬 native MySQL로 실행해 확정: tests=6 failures=0 errors=0. 11.4 반영 후 재실행으로도 재확인)
- [x] 11.8 `stats/patient-count-by-gender-age-band.html` 리스킨. **완료 기준**: `StatsViewControllerIT` 무수정 통과. (오케스트레이터가 로컬 native MySQL로 실행해 확정: tests=6 failures=0 errors=0. 11.4 반영 후 재실행으로도 재확인)
