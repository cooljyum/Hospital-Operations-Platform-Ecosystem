...
- Badges, tags, and status labels stay on a neutral `{colors.surface}` fill — colour is carried only in text, never a filled chip
- No emoji anywhere in the interface — all iconography is **SVG** (a line-icon set for UI chrome, the sticker illustration set for marketing/decoration), never an emoji glyph
...
- No filled-colour variant exists in this system: status/category/severity signal is carried entirely by text colour (and, where needed, a small coloured dot) against the neutral white fill — never a solid accent-colour chip. This applies to every label-like surface (badges, tags, status pills, table chips), not just `badge-pill` itself.
...
- Keep every badge, tag, and status label on the neutral `{colors.surface}` fill per `badge-pill` — carry the signal in text colour (or a small dot) only.
- Communicate status and meaning through type or SVG icons (line-icon set for UI chrome, sticker illustrations for decoration) — never emoji.
- Author every icon as inline SVG (or an SVG sprite/component), sized and coloured with the same tokens as surrounding type (`currentColor` for line icons) so it scales crisply at any density.
...
- Don't fill badges, pills, tags, or status labels with a solid accent-colour background (no red/green/yellow status chips) — the fill stays `{colors.surface}`, colour lives in the text only.
- Don't use emoji as icons, bullets, or inline content anywhere in the UI or copy — every glyph is SVG (line-icon set or sticker illustration), never an emoji character.
- Don't ship icons as raster images (PNG/JPG icon fonts) or emoji-font glyphs — SVG only, so icons stay theme-able and crisp at any scale.
