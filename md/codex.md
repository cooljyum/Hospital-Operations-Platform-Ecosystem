...
>
> 이 프로젝트는 2026-07-24부로 검증 파이프라인을 (기존 `docs/agents/gemini.md` 기반에서) 이 문서
> 기반으로 전환했다. `gemini.md`는 과거 기록으로 보존하되, `CLAUDE.md`의 "작업 검증 정책"이 가리키는
> 활성 파이프라인은 이제 이 문서다.
...
4. **닫힌 루프 검증**: 산출물이 다시 입력이 되는 기능(예: 배치가 만든 캐시를 다시 조회 API가 읽는 구조,
   봇 응답을 다시 입력으로 되먹이는 구조)은 실제로 왕복 테스트한다. 자기모순은 여기서만 잡힌다.
...
설치 경로는 환경마다 다르다. 이 프로젝트의 개발 머신에서는 `codex-cli 0.144.5`가
`C:\Users\주요미\AppData\Local\Programs\OpenAI\Codex\bin\codex.exe`에 설치·인증돼 있음을 2026-07-24
직접 확인했다(세션 resume 스팟체크 포함, §6 참고).

---

## 2. 표준 호출 & 세션 resume (Windows / PowerShell)

> ⚠️ **이 프로젝트의 실행 환경은 PowerShell이다.** 아래 예시는 원본(bash 환경) 문서의 `< /dev/null`
> stdin-닫기 트릭을 **쓰지 않는다** — PowerShell에서 `<`는 예약 연산자라 `ParserError`로 즉시 실패한다
> (`The '<' operator is reserved for future use.`, 2026-07-24 이 저장소에서 직접 재현·확인함).
> 이 프로젝트가 쓰는 PowerShell 실행 도구는 **stdin이 이미 null device에 연결돼 있어** 별도 처리 없이도
> stdin 대기 없이 즉시 반환된다 — 아래 예시처럼 리다이렉트 없이 그대로 호출하면 된다.
...
- 실측(이 저장소, codex-cli 0.144.5, 2026-07-24): 같은 세션 ID로 resume 시 이전 라운드 내용을
  정확히 기억했다(비밀 코드 기억 테스트, 세션 id `019f8f7e-68ca-7bc3-8d12-9893055b28e6` — 첫 호출
  2,145 tokens, resume 2,798 tokens, 맥락 재로딩 없이 정확한 답변 확인).

**모델·강도 고정값**

| 항목 | 값 | 의미 |
|------|-----|------|
| 모델 | `gpt-5.6-terra` | 이 계정에서 확인된 사용 가능 모델. 계정별로 다르면 교체 |
...
### 2c. git 안전망 (구현 라운드마다, PowerShell)

```powershell
$codexPre = git status --porcelain              # 구현 라운드 전 baseline
codex exec resume <ID> ...                       # 구현/수정 실행
$codexPost = git status --porcelain
Compare-Object $codexPre $codexPost              # 델타 = codex가 만든 변경
```
...
- b. `git diff`(또는 병렬 시 `git diff HEAD -- <대상 파일들>`)로 **실제 코드를 직접 읽고**
  완료 기준·요구사항 전 항목 대조.
- c. 빌드/테스트 **Claude가 직접 실행**한다(이 프로젝트는 `.\gradlew.bat`). 테스트는 대상 범위만
  좁혀서 돌린다.
...
동시성·보안·인증·결제·상태머신·데이터 마이그레이션 등 **틀리면 비싼 step**(이 프로젝트에서는 특히
RBAC/break-glass, 암호화·키 회전, 감사 로그, 승인 상태머신, DB 마이그레이션류)은 ④ Claude 판정에
더해, **구현 세션과 다른 새 codex 세션**에 적대 검증을 시킨다(구현자와 검증자의 세션 분리로 교차검증
확보, 토큰은 여전히 codex 쪽에서 소모):
...
> codex 실행 후 다수의 파일 변경이 보였지만 알고 보니 사용자가 별도로 진행한 무관한 작업(다른
> 기능/문서 정리)이었던 사례가 있었다. step 대상 파일이 아니면 스코프 밖 → codex 무관. 이런 걸
> codex 탓으로 오인하지 말 것.
...
| ② 공유 핫 파일 선점 | 여러 버킷이 공통으로 append해야 하는 파일(예: 이 프로젝트의 `ACCESS_POLICY_RULES` 마이그레이션, `layout/default.html` 사이드바, `SyncJobConfig`의 step 등록부)은 병렬 대상에서 **제외**하고, 선행 순차 step에서 필요한 항목을 **미리 전부 등록**한다. 이후 병렬 에이전트에겐 읽기 전용. |
...
4. 빌드/테스트 게이트를 오케스트레이터가 한 번 더 돌린다(`.\gradlew.bat test`).
...
| 증상 | 원인 | 대응 |
|------|------|------|
| `ParserError: The '<' operator is reserved for future use.` | bash용 `< /dev/null` 리다이렉트를 PowerShell에 그대로 씀 | **쓰지 않는다.** 이 프로젝트의 PowerShell 실행 도구는 stdin이 이미 null device라 리다이렉트 없이 바로 호출 |
| stdin 대기 메시지 후 무한 대기(bash 등 다른 셸 환경일 때) | stdin 미닫힘 | 그 환경에서는 `< /dev/null` 추가(이 프로젝트의 PowerShell 도구에서는 해당 없음) |
...
## 6. 검증된 예시 (이 저장소에서 실측, codex-cli 0.144.5, 2026-07-24)

```powershell
# ① 첫 호출 — 세션 ID가 배너에 찍힌다
codex exec --skip-git-repo-check --dangerously-bypass-approvals-and-sandbox `
  -m gpt-5.6-terra -c model_reasoning_effort="medium" `
  "읽기 전용 확인, 파일 수정 금지. 기억 테스트다. 비밀 코드는 'MANGO-42'다. 마지막 줄에 'ACK'만 출력하라."
# → session id: 019f8f7e-68ca-7bc3-8d12-9893055b28e6 / tokens 2,145

# ② 그 ID로 resume — 이전 라운드 맥락을 기억한다
codex exec resume 019f8f7e-68ca-7bc3-8d12-9893055b28e6 `
  --skip-git-repo-check --dangerously-bypass-approvals-and-sandbox `
  -m gpt-5.6-terra -c model_reasoning_effort="medium" `
  "아까 기억하라고 한 비밀 코드가 뭐였지? 마지막 줄에 'CODE: <값>'만 출력하라."
# → CODE: MANGO-42 (정확) / tokens 2,798 — 맥락 재로딩 비용 없음
```
...
codex exec --skip-git-repo-check --dangerously-bypass-approvals-and-sandbox `
  "읽기 전용 확인. 수정 금지. <경로>가 존재하는가?
   마지막 줄에 'RESULT: <PRESENT|ABSENT>' 만 출력하라."
```
